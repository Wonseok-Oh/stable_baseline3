...
def get_meta_action_dim(meta_action_space: spaces.Space) -> int:
    """
    Get the dimension of the action space.

    :param meta_action_space:
    :return:
    """
    if isinstance(meta_action_space, spaces.Box):
        return int(np.prod(meta_action_space.shape))
    elif isinstance(meta_action_space, spaces.Discrete):
        # Action is an int
        return 1
    elif isinstance(meta_action_space, spaces.MultiDiscrete):
        # Number of discrete actions
        return int(len(meta_action_space.nvec))
    elif isinstance(meta_action_space, spaces.MultiBinary):
        # Number of binary actions
        return int(meta_action_space.n)
    else:
        raise NotImplementedError(f"{meta_action_space} meta action space is not supported")
