...
class VecEnv(ABC):
    ...
    def __init__(self, num_envs: int, observation_space: gym.spaces.Space, action_space: gym.spaces.Space, meta_action_space: gym.spaces.Space):
    ...
    @abstractmethod
    def invoke_async(self, meta_actions: np.ndarray) -> None:
        """
        Tell all the environments to start taking a step
        with the given actions.
        Call invoke_wait() to get the results of the step.

        You should not call this if a invoke_async run is
        already pending.
        """
        raise NotImplementedError()

    @abstractmethod
    def invoke_wait(self) -> VecEnvStepReturn:
        """
        Wait for the invoke taken with invoke_async().
    ...
    def invoke(self, meta_actions: np.ndarray) -> VecEnvStepReturn:
        """
        Step the environments with the given meta_action

        :param meta_actions: the meta_action
        :return: observation, reward, done, information
        """
...
class VecEnvWrapper(VecEnv):
    ...
    def __init__(
        ...
        meta_action_space: Optional[gym.spaces.Space] = None
    ):
        self.venv = venv
        VecEnv.__init__(
            self,
            ...
            meta_action_space=meta_action_space or venv.meta_action_space
        )
    ...
    def invoke_async(self, meta_actions: np.ndarray) -> None:
        self.venv.invoke_async(meta_actions)
